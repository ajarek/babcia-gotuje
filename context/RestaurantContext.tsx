...
interface RestaurantContextType {
  ...
  createReservation: (date: string, time: string, guests: number, phone: string, notes?: string) => Promise<string>;
...
const INITIAL_MENU: Omit<MenuItem, 'id'>[] = [
  {
    ...
    description: 'Tradycyjny, gęsty żurek na domowym zakwasie, z białą kiełbasą, jajkiem, majerankiem i boczkiem, podawany w chrupiącym chlebku rzemieślniczym.',
    price: 24,
    category: 'soups',
  ...
  },
  {
    ...
    description: 'Aromatyczny, czysty barszcz z buraków kiszonych według dawnej receptury, serwowany z ręcznie lepionymi uszkami z farszem z grzybów leśnych.',
    price: 22,
    category: 'soups',
  ...
  {
    name: 'Pierogi Ruskie',
    description: 'Ręcznie lepione pierogi z farszem z delikatnego twarogu, ugotowanych ziemniaków i zeszklonej cebulki. Podawane z chrupiącą okrasą.',
    price: 26,
    category: 'mains',
  ...
  },
  {
    ...
    description: 'Tradycyjne polskie pierogi faszerowane aromatycznym, długo duszonym mięsem wieprzowo-wołowym z dodatkiem cebulki i majeranku.',
    price: 28,
    category: 'mains',
  ...
  },
  {
    ...
    description: 'Klasyczny, soczysty kotlet schabowy w chrupiącej złocistej panierce, smażony na tradycyjnym smalcu. Serwowany z ziemniaczkami z koperkiem i kapustą zasmażaną.',
    price: 38,
    category: 'mains',
  ...
  },
  {
    ...
    description: 'Liście sparzonej kapusty faszerowane soczystym mięsem mielonym oraz ryżem, duszone w domowym, aksamitnym sosie ze świeżych pomidorów.',
    price: 32,
    category: 'mains',
  ...
  },
  {
    ...
    description: 'Trzy duże, wybitnie chrupiące placki ziemniaczane przełożone sycącym, gęstym gulaszem wołowym z warzywami, udekorowane kleksem kwaśnej śmietany.',
    price: 39,
    category: 'mains',
  ...
  },
  {
    ...
    description: 'Niezwykle puszysty, pieczony na miejscu sernik z prawdziwego, tłustego twarogu z dodatkiem rodzynek sułtańskich i kandyzowanej skórki pomarańczowej.',
    price: 18,
    category: 'desserts',
  ...
  },
  {
    ...
    description: 'Kruche ciasto wypełnione po brzegi aromatycznymi, lekko kwaśnymi jabłkami z polskich sadów z nutą cynamonu, podawane na ciepło z gałką lodów waniliowych.',
    price: 19,
    category: 'desserts',
  ...
  },
  {
    ...
    description: 'Eko kompot gotowany codziennie na bazie świeżych i suszonych owoców sezonowych (jabłka, gruszki, truskawki) z lekką nutą goździków. Podawany na zimno.',
    price: 8,
    category: 'drinks',
...
export function RestaurantProvider({ children }: { children: React.ReactNode }) {
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            const role = currentUser.email === 'ajarek2101@gmail.com' ? 'admin' : 'user';
        ...
        const storedLocalUser = localStorage.getItem('babcia_current_local_user');
        if (storedLocalUser) {
          try {
    ...
    const fetchAndSeedMenu = async () => {
      try {
        ...
        if (querySnapshot.empty) {
          // Seed the menu
          console.log('Menu jest puste. Rozpoczynam automatyczne dodawanie dań...');
          ...
          querySnapshot.forEach((doc) => {
            ...
            items.push({
              ...
              image: data.image || `https://picsum.photos/seed/${data.name}/800/600`,
  ...
  useEffect(() => {
    ...
    const unsubRes = onSnapshot(resQuery, (snapshot) => {
      ...
      fetchedRes.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  useEffect(() => {
    ...
    const unsubAllRes = onSnapshot(collection(db, 'reservations'), (snapshot) => {
  ...
  const createOrder = async (address: string, phone: string, notes?: string) => {
  ...
  const createReservation = async (date: string, time: string, guests: number, phone: string, notes?: string) => {
    const resData: Omit<Reservation, 'id'> = {
      userId: user ? user.uid : null,
      userName: user ? (user.displayName || 'Klient') : 'Gość',
      userEmail: user ? (user.email || 'gosc@babciagotuje.pl') : 'gosc@babciagotuje.pl',
      date,
      time,
      guests,
      phone,
  ...
  const loginWithGoogle = async () => {
    try {
      ...
      console.warn("Google Sign-In failed or is disabled. Using local fallback...", err);
      ...
      localStorage.setItem('babcia_current_local_user', JSON.stringify(mockGoogleUser));
  ...
  const loginWithEmail = async (email: string, pass: string) => {
    try {
      ...
      if (err.code === 'auth/operation-not-allowed' || err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password') {
        const localUsers = JSON.parse(localStorage.getItem('babcia_local_users') || '[]');
        const foundUser = localUsers.find((u: any) => u.email === email && u.password === pass);
        
        if (foundUser) {
          localStorage.setItem('babcia_current_local_user', JSON.stringify(foundUser));
        ...
        if (email === 'ajarek2101@gmail.com') {
          ...
          localStorage.setItem('babcia_current_local_user', JSON.stringify(adminUser));
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed' || err.code === 'auth/admin-restricted-operation') {
        ...
        if (localUsers.find((u: any) => u.email === email)) {
          throw { code: 'auth/email-already-in-use', message: 'Ten adres e-mail jest już zajęty przez innego wnuczka!' };
        }
        ...
        localStorage.setItem('babcia_local_users', JSON.stringify(localUsers));
        localStorage.setItem('babcia_current_local_user', JSON.stringify(newUser));
