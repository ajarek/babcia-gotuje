...
import { X, Calendar, Clock, Users, Phone, MessageSquare, CheckCircle } from 'lucide-react';
...
export default function ReservationModal({ isOpen, onClose, onOpenAuth }: ReservationModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError('Przepraszamy, nie udało się zapisać rezerwacji. Spróbuj zadzwonić bezpośrednio do lokalu!');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="p-8 pt-10">
          {success ? (
            <div className="text-center py-8 flex flex-col items-center justify-center">
              ...
              <h3 className="font-serif text-3xl font-bold text-emerald-950">Stolik zarezerwowany!</h3>
              <p className="text-sm text-amber-950/80 mt-3 max-w-sm leading-relaxed">
                Rezerwacja została pomyślnie zapisana. Przygotowujemy dla Was najprzytulniejszy kącik w naszej karczmie!
              </p>
              ...
              {!user && (
                <p className="text-xs text-[#78350F] font-semibold mt-4">
                  Zarezerwowałeś jako Gość. <button onClick={() => { closeAndReset(); onOpenAuth(); }} className="underline hover:text-[#5C230A]">Załóż konto</button>, aby zbierać darmowe desery przy kolejnych rezerwacjach!
                </p>
              )}
              <button
                onClick={closeAndReset}
            ...
            <>
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-amber-950/70 mt-1.5">
                  Zjedz u nas obiad, kolację lub zorganizuj rodzinne przyjęcie u Babci.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Date */}
                  <div>
                    <label className="block text-xs font-semibold text-amber-900/80 uppercase tracking-wider mb-1 flex items-center gap-1.5">
                ...
                <div>
                  <label className="block text-xs font-semibold text-amber-900/80 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
