...
import { X, Trash2, ShoppingBag, MapPin, Phone, MessageSquare, ClipboardCheck } from 'lucide-react';
...
export default function CartSheet({ isOpen, onClose, onOpenAuth }: CartSheetProps) {
  const { cart, updateQuantity, removeFromCart, createOrder, user } = useRestaurant();
  ...
  const handleOrderSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError('Ojej, nie udało się złożyć zamówienia. Spróbuj ponownie lub zadzwoń do nas!');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <div className="px-6 py-5 bg-[#FAF6F0] border-b border-amber-900/10 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="font-serif text-2xl font-bold text-[#4A2E1A]">Twój Koszyk</h2>
            </div>
            <button 
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {successOrderId ? (
              <div className="text-center py-12 px-4 flex flex-col items-center justify-center h-full">
                ...
                <h3 className="font-serif text-2xl font-bold text-emerald-950">Dziękujemy za zamówienie!</h3>
                <p className="text-sm text-amber-950/80 mt-3 max-w-xs leading-relaxed">
                  Twoje zamówienie zostało pomyślnie złożone w naszej kuchni. Babcia już stoi przy garach!
                </p>
                ...
                {!user && (
                  <p className="text-xs text-amber-950/60 mb-6 max-w-xs">
                    Złożyłeś zamówienie jako Gość. <button onClick={() => { closeAndReset(); onOpenAuth(); }} className="text-[#78350F] font-semibold underline hover:text-[#5C230A]">Zarejestruj się</button>, aby śledzić status swoich zamówień i zbierać punkty!
                  </p>
                )}
                <button
                  onClick={closeAndReset}
              ...
              <div className="text-center py-16 flex flex-col items-center justify-center h-full">
                ...
                <h3 className="font-serif text-lg font-bold text-amber-950/80">Twój koszyk jest pusty</h3>
                <p className="text-sm text-amber-950/60 mt-1 max-w-xs">
                  Ojej, nic nie wybrałeś! Dodaj jakieś pyszności z menu Babci, na pewno jesteś głodny.
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-bold text-amber-900/60 uppercase tracking-widest">Wybrane pyszności</h3>
                      ...
                      <div key={item.id} className="py-3 flex gap-3.5">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between font-medium">
                            <h4 className="text-[#4A2E1A] font-medium text-sm leading-snug">{item.name}</h4>
                            <span className="text-sm text-[#78350F] font-bold">{item.price * quantity} zł</span>
                          </div>
                          <p className="text-xs text-amber-950/60 mt-0.5">{item.price} zł / szt.</p>
                ...
                <form onSubmit={handleOrderSubmit} className="space-y-4 pt-4 border-t border-amber-900/10">
                  <h3 className="text-xs font-bold text-amber-900/60 uppercase tracking-widest">Szczegóły dostawy</h3>
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-medium text-amber-900/80 mb-1 flex items-center gap-1.5">
                  ...
                  <div className="pt-4 border-t border-amber-900/10 space-y-3">
                    <div className="flex justify-between text-amber-950/80 text-sm">
                      <span>Dostawa:</span>
                      <span className="font-semibold text-emerald-700">Darmowa dostawa!</span>
                    </div>
                    <div className="flex justify-between text-base font-bold text-[#4A2E1A] pt-1">
                      <span>Razem:</span>
                      <span className="font-serif text-lg">{totalPrice} zł</span>
                    </div>
