...
import { useRestaurant, Order, Reservation, MenuItem } from '@/context/RestaurantContext';
import { 
  ClipboardList, 
  CalendarCheck, 
  PlusCircle, 
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'reservations' | 'add-item'>('orders');
  
  // Custom Menu Item Form State
  const [itemName, setItemName] = useState('');
  const [itemDescription, setItemDescription] = useState('');
  const [itemPrice, setItemPrice] = useState('');
  const [itemCategory, setItemCategory] = useState<'soups' | 'mains' | 'desserts' | 'drinks'>('mains');
  ...
  const activeReservationsCount = allReservations.filter(r => r.status === 'pending').length;
  ...
  const handleAddMenuItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const defaultImage = itemImage || `https://picsum.photos/seed/${itemName}/800/600`;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-amber-900/10 overflow-hidden font-sans text-[#2D241E] max-w-6xl mx-auto p-6 sm:p-8">
      ...
      <div className="border-b border-amber-900/10 pb-6 mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <span className="px-3 py-1 bg-amber-100 text-[#78350F] rounded-full text-xs font-bold uppercase tracking-wider">PANEL ZARZĄDZANIA</span>
          <h2 className="font-serif text-3xl font-bold text-[#4A2E1A] mt-2">Babcia Gotuje • Administracja</h2>
          <p className="text-sm text-amber-950/60 mt-1">Zarządzaj rezerwacjami stolików, zamówieniami klientów i asortymentem menu w czasie rzeczywistym.</p>
        </div>
        
        {/* Quick Stats Grid */}
        <div className="grid grid-cols-3 gap-3">
          <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100 text-center">
            <p className="text-[10px] text-emerald-800 uppercase tracking-widest font-semibold">Utarg (dostarczone)</p>
            <p className="text-sm sm:text-base font-bold text-emerald-950 mt-0.5">{totalSales} zł</p>
          </div>
          <div className="p-3 bg-amber-50 rounded-xl border border-amber-100 text-center">
            <p className="text-[10px] text-amber-800 uppercase tracking-widest font-semibold">Zamówienia</p>
            <p className="text-sm sm:text-base font-bold text-amber-950 mt-0.5">{pendingOrdersCount} oczk.</p>
          </div>
          <div className="p-3 bg-indigo-50 rounded-xl border border-indigo-100 text-center">
            <p className="text-[10px] text-indigo-800 uppercase tracking-widest font-semibold">Rezerwacje</p>
            <p className="text-sm sm:text-base font-bold text-indigo-950 mt-0.5">{activeReservationsCount} oczk.</p>
          </div>
        </div>
      </div>
      ...
      <div>
        {activeTab === 'orders' && (
          <div className="space-y-4">
            {allOrders.length === 0 ? (
              <div className="text-center py-12 text-amber-950/40">Brak zamówień w systemie.</div>
            ) : (
                    ...
                    {allOrders.map((order) => (
                      <tr key={order.id} className="hover:bg-amber-500/5 transition">
                        <td className="p-4 font-semibold text-[#4A2E1A]">
                          ...
                          <div className="text-xs font-normal text-amber-950/50">{order.userEmail}</div>
                          <div className="text-[10px] font-mono text-amber-950/40 mt-1">ID: {order.id.substring(0, 8)}...</div>
                        </td>
                        <td className="p-4 max-w-[220px]">
                          <div className="space-y-1">
                            {order.items.map((it, i) => (
                              <div key={i} className="text-xs">
                                <span className="font-semibold text-[#78350F]">{it.quantity}x</span> {it.name}
                              </div>
                            ))}
                          </div>
                          {order.notes && (
                        ...
                        <td className="p-4 font-bold text-[#78350F]">{order.total} zł</td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${
                            ...
                            order.status === 'preparing' ? 'bg-indigo-100 text-indigo-800 animate-pulse' :
                            order.status === 'delivered' ? 'bg-emerald-100 text-emerald-800' :
                            'bg-red-100 text-red-800'
                          }`}>
                            {order.status === 'pending' ? 'Nowe' :
                             order.status === 'preparing' ? 'Kuchnia' :
        ...
        {activeTab === 'reservations' && (
          <div className="space-y-4">
            {allReservations.length === 0 ? (
              <div className="text-center py-12 text-amber-950/40">Brak rezerwacji w systemie.</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[700px]">
                  <thead>
                  ...
                  <tbody className="divide-y divide-amber-900/5 text-sm">
                    {allReservations.map((res) => (
                      <tr key={res.id} className="hover:bg-amber-500/5 transition">
                        <td className="p-4 font-semibold text-[#4A2E1A]">
                          <div>{res.userName}</div>
                          <div className="text-xs font-normal text-amber-950/50">{res.userEmail}</div>
                        </td>
                        <td className="p-4">
                          <div className="font-semibold text-[#4A2E1A]">{res.date}</div>
                          <div className="text-xs text-[#78350F] font-bold">o godz. {res.time}</div>
                        </td>
                        <td className="p-4 font-bold">
                          {res.guests} {res.guests === 1 ? 'osoba' : res.guests < 5 ? 'osoby' : 'osób'}
                        </td>
                        ...
                        <td className="p-4">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${
                            ...
                            {res.status === 'pending' ? 'Oczekuje' :
                             res.status === 'confirmed' ? 'Zatwierdzona' :
                             'Anulowana'}
                          </span>
                        </td>
                        <td className="p-4 text-right">
          ...
          <div className="max-w-2xl mx-auto">
            ...
            {submitSuccess && (
              <div className="p-4 mb-6 text-sm text-emerald-800 bg-emerald-50 rounded-xl border border-emerald-200">
                Wspaniale! Danie zostało pomyślnie dodane do bazy danych i jest widoczne dla klientów!
              </div>
            )}
            ...
            <form onSubmit={handleAddMenuItem} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-amber-900/80 uppercase tracking-wider mb-1">Nazwa dania <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                    value={itemName}
                ...
                <div>
                  <label className="block text-xs font-semibold text-amber-900/80 uppercase tracking-wider mb-1">Cena (PLN) <span className="text-red-500">*</span></label>
                  <input
                    type="number"
                    required
                    value={itemPrice}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-amber-900/80 uppercase tracking-wider mb-1">Kategoria dania <span className="text-red-500">*</span></label>
                  <select
                    value={itemCategory}
                ...
                <div>
                  <label className="block text-xs font-semibold text-amber-900/80 uppercase tracking-wider mb-1">URL Obrazka (Opcjonalnie)</label>
                  <input
                    type="url"
                    value={itemImage}
              ...
              <div>
                <label className="block text-xs font-semibold text-amber-900/80 uppercase tracking-wider mb-1">Opis dania <span className="text-red-500">*</span></label>
                <textarea
                  required
